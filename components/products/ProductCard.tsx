...
export default function ProductCard({ product, onEdit, onDelete }: ProductCardProps) {
  ...
  return (
    <Card className="group relative overflow-hidden border-0 shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 bg-white">
      ...
      <CardContent className="pt-6 relative">
        ...
        <div className="aspect-square bg-gradient-to-br from-gray-100 to-gray-200 rounded-2xl mb-4 flex items-center justify-center overflow-hidden shadow-inner group-hover:shadow-xl transition-all duration-300">
          {product.image ? (
            <img
              src={product.image}
              alt={product.name}
            ...
            <div className="relative">
              ...
              <Package className="w-20 h-20 text-gray-400 relative" />
            </div>
          )}
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <h3 className="font-bold text-xl line-clamp-1 bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent group-hover:from-blue-600 group-hover:to-purple-600 transition-all duration-300">
                {product.name}
              </h3>
          ...
          {/* Prix et Marge - DESIGN PREMIUM */}
          <div className="pt-3 bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 p-4 rounded-xl space-y-2 border-2 border-blue-100 shadow-inner">
            <div className="flex items-center justify-between text-xs">
              <span className="text-gray-600 font-medium flex items-center gap-1">
                <span className="w-2 h-2 bg-gray-400 rounded-full"></span>
                Prix d'achat
              </span>
              <span className="font-semibold text-gray-700 bg-white px-2 py-0.5 rounded-md shadow-sm">
                {purchasePrice.toFixed(2)} DH
              </span>
            </div>
            ...
            <div className="pt-2 border-t-2 border-white/50">
              <div className="flex items-center justify-between">
                <span className="font-semibold text-gray-700 flex items-center gap-1">
                  <span className={`w-2 h-2 rounded-full ${marginPercentage >= 30 ? 'bg-green-500' : marginPercentage >= 15 ? 'bg-orange-500' : 'bg-red-500'}`}></span>
                  Marge
                </span>
                <div className="text-right">
                  <div className={`inline-flex items-center gap-1 px-3 py-1 rounded-full font-bold text-sm shadow-md ${
                    marginPercentage >= 30
      ...
      <CardFooter className="flex gap-2 pt-0 pb-6">
        <Button
          variant="outline"
          size="sm"
          className="flex-1 border-2 border-blue-200 hover:border-blue-400 hover:bg-blue-50 transition-all duration-300 shadow-sm hover:shadow-md"
